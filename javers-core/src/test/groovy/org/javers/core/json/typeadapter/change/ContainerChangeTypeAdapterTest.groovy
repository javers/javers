...
class ContainerChangeTypeAdapterTest extends Specification {
    ...
    def "should deserialize #changeType_.simpleName with references"() {
        ...
        json {
            leftValue({ value new SnapshotEntity(id: 1) }, { value new SnapshotEntity(id: 2) })
            rightValue({ value new SnapshotEntity(id: 1)  }, { value new SnapshotEntity(id: 3)  })
    ...
    @Unroll
    def "should deserialize #changeType_.simpleName with left and right values"() {
        given:
        def javers = javersTestAssembly()
        JsonConverter jsonConverter = javers.jsonConverter
        def snapshot1 = new SnapshotEntity(id: 1)
        def snapshot2 = new SnapshotEntity(id: 2)
        def snapshot3 = new SnapshotEntity(id: 3)
        def json = new JsonBuilder()
        json {
            leftValue({ value snapshot1 }, { value snapshot2 })
            rightValue({ value snapshot1  }, { value snapshot3  })
            changeType changeType_.simpleName
            globalId {
                entity "org.javers.core.model.SnapshotEntity"
                cdoId 1
        ...
        change.propertyName == propertyName
        change.getAffectedGlobalId() == instanceId(1, SnapshotEntity)
        if (changeType_ == ListChange) {
            assert ((ListChange) change).left.contains(snapshot1)
            assert ((ListChange) change).left.contains(snapshot2)
            assert !((ListChange) change).left.contains(snapshot3)
            assert ((ListChange) change).right.contains(snapshot1)
            assert !((ListChange) change).right.contains(snapshot2)
            assert ((ListChange) change).right.contains(snapshot3)
        } else {
            assert ((ArrayChange) change).left.contains(snapshot1)
            assert ((ArrayChange) change).left.contains(snapshot2)
            assert !((ArrayChange) change).left.contains(snapshot3)
            assert ((ArrayChange) change).right.contains(snapshot1)
            assert !((ArrayChange) change).right.contains(snapshot2)
            assert ((ArrayChange) change).right.contains(snapshot3)
        }
    ...
    @Unroll
    def "should serialize #changeType.simpleName with left and  right values"() {
        given:
        def javers = javersTestAssembly()
        def cdo = new SnapshotEntity(id: 1)
        def ref2 = javers.instanceId(new SnapshotEntity(id: 2))
        ...
        def json = new JsonSlurper().parseText(jsonText)
        json.property == propertyName
        with(json.leftValue[0]){
            value == 1
        }
        with(json.leftValue[1]){
            value == 2
        }
        with(json.rightValue[0]){
            value == 3
        }
        with(json.rightValue[1]){
            value == 4
        }
        json.changeType == changeType.simpleName
        json.globalId
    ...
    def "should deserialize #changeType_.simpleName with #javersType"() {
        ...
        def json = new JsonBuilder()
        json {
            leftValue([{ value  val1given }])
            rightValue([{ value val2given }])
            changeType changeType_.simpleName
            globalId {
                entity "org.javers.core.model.SnapshotEntity"
                cdoId 1
    ...
    def "should deserialize SetChange with Primitives"() {
        ...
        def json = new JsonBuilder()
        json {
            leftValue({ value 1 }, { value 2 })
            rightValue({ value 3 }, { value 4 })
