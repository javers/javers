...
class MultisetChangeAppenderTest extends AbstractDiffAppendersTest {
    ...
    def "should throw exception on different multiset implementation"(){
        when:
        buildGraph(new SnapshotEntity(multiSetValueObject: HashMultiset.create([new DummyAddress(city: "New York")])))
        buildGraph(new SnapshotEntity(multiSetValueObject: TreeMultiset.create([new DummyAddress(city: "New York")])))


        then:
        thrown(ClassCastException) //todo properly handle exception
    }
